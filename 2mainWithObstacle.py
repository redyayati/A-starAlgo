# A* algorithm 
# Adding obstacles and diagonal path neighbors 
# Also, imporovement is that, in the main loop, current node is assigned as previous only if the node has gotten better
import pygame as pg 
...
class Spot : 
    ...
    def show(self, color) : 
        x = self.i * w
        y = self.j * h 
        if self.wall : color = (100,100,100)
        pg.draw.rect(screen, (0,0,0), (x,y,w,h), 1)
...
while running : 
    ...
    if n == 0 : # This is to stop the loop when its done 
        if len(openSet) > 0 :  # As long as there are nodes to evaluate in openSet, keep going 
            ...
            for neighbor in neighbors : 
                    if neighbor not in closedSet and not neighbor.wall: # examin only when it is not in closedSet and not a wall
                        tempG = current.g + 1 
                        betterNode = False
    ...
    for i in range(rows) :
        for j in range(cols) :  
            grid[i][j].show((150,150,190)) # All squares in grid are shown in this color
    path = []
    temp = current
    path.append(temp)
    while temp.previous != None : 
        path.append(temp.previous) # adding previous parent to the path array to generate path 
        temp = temp.previous
    for closed in closedSet : closed.show((190,150,150)) # All squares in closedSet are in this color
